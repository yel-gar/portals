...
Компоненты `creatures_count` и `ttl` пропущены через насыщающуюся функцию вида `x / (x + 1)` вместо линейной - это намеренно: разница между 0 и 5 существами внутри портала должна значить куда больше для риска, чем разница между 50 и 55, аналогично для оставшегося времени до схлопывания. Линейная формула дала бы вклад, растущий непропорционально для больших значений; насыщение ограничивает вклад каждого компонента сверху, оставляя формулу интерпретируемой (итоговый риск всегда в `[0, 1]`) и не давая одному экстремальному параметру полностью забить остальные три. Итоговое значение маппится в уровни: `0.0–0.3` - low, `0.3–0.6` - medium, `0.6–0.85` - high, `> 0.85` - critical.
...
- Плотность: два слоя пыли-глиттера (сетка 260/520, пороги 0.35/0.45) - сотни тысяч мелких точек; яркость поднята.
- Финальный фидбек: `The particles on wgpu are flickering too much. Remove particles thing and just keep those orange 'flames'.` Слой частиц удалён целиком, оставлены только потоки.

### Итоговое фактическое состояние шейдера
В `wgpuFire.ts` остался только потоковый эффект - ни одного слоя частиц, точек или бликов:
- два потока (A и B) - `valueNoise` по вытянутому вдоль x домену, `smoothstep`-полоса и её квадрат дают тонкие текучие струи; они складываются с тёмной эмбер-подложкой (`vec3(0.026, 0.01, 0.004)`, дышит от общей турбулентности) и эллиптическим затуханием к краям;
- общий для обоих потоков `valueNoise`-турбулентность и медленный крупномасштабный swirl вокруг центра экрана (cos через сдвиг sin) закручивают струи;
- сохранены sin-free хэш (`fract`-каскад), `alphaMode: "opaque"`, `time % 3600`, пауза в скрытой вкладке, guard присваивания `canvas.width`, половинное разрешение и обработка `device.lost`.
Живые ручки (`fireTuning`) сократились до `flowA`, `flowB`, `swirl`, `warpFreq`, `bright`; пылевые `cutA`/`cutB`/`dotScale` и vec4-униформа `tuneB` (схлопнута в скаляр `bright`) удалены. 2D-canvas-фолбэк (`emberLayer.ts`) не тронут и по-прежнему рисует собственные 140 искр - это отдельный, дешёвый рендерер.

### Итоговая политика
Шейдер потоков стал фоном по умолчанию везде, где есть GPU, включая Windows (временное правило "на Windows всегда ember" отменено). Откатная лестница сохранена: `?fire=ember`, отказ инициализации, фолбэк, `device.lost`, переинициализация. Проверено: unit 129/129, backend 134/134, e2e 38/38, визуально на реальном железе. Удаление слоя частиц тестов не меняло (у шейдера нет юнит-тестов) и проверено визуально.
