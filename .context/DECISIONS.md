...
- **Translucent surfaces over the spark field (user-directed follow-up)**: the spark layer spans the **whole shell** (sidebar included), and the main UI surfaces are **slightly transparent** (plain rgba, no blur): sidebar gradient `rgba(21,13,5,0.86)→rgba(10,6,3,0.86)`, header `rgba(18,11,7,0.84)`, cards/table/inputs via `colorBgContainer: rgba(23,16,8,0.82)`. Dialog surfaces (`colorBgElevated` — modal, dropdowns, messages) stay opaque for readability.
