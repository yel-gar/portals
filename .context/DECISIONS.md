...
- Review fixes (2026-09): the simulator's per-tick snapshot of open portals is taken with `SELECT ... FOR UPDATE` so a concurrent portal action can never be overwritten by the tick's stale flush (both writers lock the same row; the blocked one re-reads fresh data). Subscriber refresh queues are bounded (`maxsize=1`, broadcasts skipped while a refresh is pending) so a slow client cannot accumulate unbounded events. `UpdateHub.stop()` cancels the supervisor so a reconnect backoff sleep never delays shutdown. `DISABLE_REGISTRATION` is passed through `docker-compose.yml` like other settings.
