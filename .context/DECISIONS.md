...
- Review fixes (2026-09): the simulator's per-tick snapshot of open portals is taken with `SELECT ... FOR UPDATE SKIP LOCKED` — the lock keeps a concurrent portal action from being overwritten by the tick's stale flush, and `SKIP LOCKED` means a portal currently being acted on is skipped (returned to on a later tick) so actions never block behind the simulator. Subscriber refresh queues are bounded (`maxsize=1`, broadcasts skipped while a refresh is pending) so a slow client cannot accumulate unbounded events. `UpdateHub.stop()` cancels the supervisor so a reconnect backoff sleep never delays shutdown. `DISABLE_REGISTRATION` is passed through `docker-compose.yml` like other settings.
