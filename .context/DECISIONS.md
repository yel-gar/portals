...
- `order_by` is a whitelist enum validated by FastAPI (`PortalOrder` / `LogOrder`): `risk` (default), `risk_value`, `expires_at`, `creatures`, `name` for portals; `newest` (default) / `oldest` for the log. Unknown values return 422.
- Default portal ordering: open portals first (closed/expired sink below and keep their own relative order — risk and expiry estimates are meaningless for them, mirroring `/stats`), then **danger level DESC** (CRITICAL → LOW; ordered by a discrete rank, not the raw risk value), expires_at ASC, has_observer DESC, creatures_count DESC, `id ASC` as the final tiebreaker for stable pagination. Applies to every `order_by` mode (the `risk` mode's primary key is the danger level). The separate `risk_value` mode sorts by the **raw risk value DESC** (after the same open-first / dismissed-sink keys) so the risk column is monotonic among open, non-parked portals; `risk` stays the default and keeps the stable discrete danger-level buckets.
- Frontend mapping: `risk` is labelled «По умолчанию» (the danger-level order) and `risk_value` as «По риску». Portals parked by DISMISS carry an «Отложен» badge derived from `dismissed_until` so the ordering sink is visible instead of looking like a sort bug; the ordering itself is unchanged.
...
- CRITICAL portals bypass that scoring with a dedicated 100-point table: observer + creatures → WARN_CREATURES, observer without creatures → RECALL_OBSERVER, no observer → CLOSE. The 100 dominates every general condition, so a critical portal is never left as DISMISS/STABILIZE/SEND_OBSERVER. The empty-creatures row is defensive (the risk formula cannot reach CRITICAL without creatures); the no-observer row relies on the force-close rule for critical portals.
