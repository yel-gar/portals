...
- **Simulator ↔ action race (P1)**: `simulate_once()` locks its open-portal snapshot with `SELECT ... FOR UPDATE SKIP LOCKED`. The lock alone prevents the tick's flush from overwriting a concurrently committed action (it only modifies rows it locked itself); `SKIP LOCKED` additionally means a portal that is being acted on right now is skipped, so actions on unrelated portals never block behind the tick. `test_simulate_once_skips_action_locked_portal` proves the tick returns promptly while an action holds the row lock and leaves that portal's stability/creatures untouched (it can be updated on a later tick).
