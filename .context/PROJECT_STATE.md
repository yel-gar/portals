...
2. **Deps** — add `typegpu` (runtime, dynamic-imported) + `@webgpu/types` (dev, scoped imports).
3. **Fire panels** — `FirePanels` component (left/right side strips in `AppLayout`): WebGPU path (dynamic import, WGSL turbulence/fire program) with 2D-canvas ember fallback; reduced-motion → static gradient; `aria-hidden`, `pointer-events: none`, no layout shift; vitest for the feature-detection gate.
4. **FLIP** — `useFlip` hook over `PortalTable` (rect per `portal.id` before paint, WAAPI translateY on displacement; skip mount + reduced motion); vitest for delta math.
5. **Gates + e2e** — full frontend gates (typecheck/oxlint/prettier/vitest/build) + full 38-test Playwright suite stay green; smoke assertion that the panels render (fallback path in headless).
6. **Review + merge** — squash/rebase to master after green.
