...
Latest batch (portal sorting clarity, committed on master): backend — new `order_by=risk_value` mode sorts by the raw risk value DESC (after the same open-first / dismissed-sink keys), so the risk column is monotonic among open non-parked portals; `risk` stays the default discrete danger-level order (stable buckets). Frontend — the `risk` sort option is relabelled «По умолчанию», a new «По риску» option maps to `risk_value`, and portals parked by DISMISS show an «Отложен» badge from `dismissed_until` so the sink is visible instead of looking like a sort bug. Backend 143 tests; frontend 143 Vitest tests, tsc/oxlint/prettier/build green.
...
### Fire/background UX follow-up (master, 2026-09 — user-reported issues after the merge)
Four UX issues reported by the user, all fixed:
- **Full-background spark field** (was: two side fire strips with rotating ovals): the shader now covers the whole `.app-main` background — three rarefied spark-particle layers drifting upward, warped by a shared slow turbulence noise sample, dark ember base, soft elliptical edges, `premultiplied` alpha, new `resolution` vec2 uniform written on resize. The 2D-canvas fallback mirrors it with 140 rising additive ember sparks (deterministic `sparkSpec` per index), the reduced-motion static gradient now covers the full area too.
- **FLIP slowed**: `FLIP_DURATION_MS` 240 → 400 ms.
- **Modal close animation restored**: `PortalModal` stays mounted with a controlled `open={portal !== null}` — unmounting the whole component on close was cutting antd's leave motion short. Enter/close animations both play now.
- **Instant action grey-out**: a successful action feeds the response portal back (`onPortalUpdated` → `setSelectedPortal`), so a CLOSE greys out every action immediately even when the row sinks off the visible page — no waiting for the next snapshot. Covered by a PortalsPage integration test (GET refetch sees server truth).
Gates green (tsc/oxlint/prettier, 118 Vitest incl. new `sparkSpec` + grey-out/onPortalUpdated tests, build + gzip check), full 38-test Playwright suite green; SwiftShader smoke re-verified the new shader (both panels `webgpu`, zero console errors, warm spark pixels read back).

### Translucent UI over the spark field (master, 2026-09 — user request)
The background effect now frames the **whole app** instead of the content column: the spark layer moved from `.app-main` to the `.app-shell` root (spans the sidebar too), and the main UI surfaces became **transparent** (plain rgba — user chose alpha over frosted-glass blur, then asked for more transparency after the first pass): sidebar gradient `rgba(21,13,5,0.42)→rgba(10,6,3,0.42)`, header `rgba(18,11,7,0.4)`, cards/table/inputs via `colorBgContainer: rgba(23,16,8,0.45)` in `theme.ts`. Dialogue surfaces (`colorBgElevated` — modal, dropdowns, messages) are **fully opaque** (`#1e1409`): the matte-glass modal experiment (backdrop blur + lighter mask) still read as a solid panel, so per the user it was dropped — dialogs stay solid for readability. Two follow-up fixes: the pinned «Портал»/«Детали» table columns got an **opaque background** (`#171008`, `!important`) so at narrow widths the columns scrolling underneath no longer show through the sticky cells (the translucent container token had made them see-through); stats polling dropped from 20 s to **10 s** so the stats page refreshes near-real-time while mounted. Gates green so far: tsc/oxlint/prettier, 119 Vitest, build; translucency + overlap lived checked on the dev stack (fixed cells opaque even under row hover, modal solid, /stats requested at 10 s intervals).

### Page transitions (master, 2026-09 — user request)
User-requested: animate the switch between pages — "each element popping up with slight jitter offset with bouncy animation". Implemented **CSS-only**: `AppLayout` wraps the routed subtree (and the header title/subtitle) in `key={location.pathname}` containers, so every navigation + first load remounts the page and replays the entrance. Each top-level block pops with an overshoot curve (`cubic-bezier(0.34, 1.56, 0.64, 1)`, 0.42 s, `backwards` fill), a per-`nth-child` jittered rise/rotation (`--rise`/`--jitter`, e.g. ±0.7° and 9–18 px) and a 0–180 ms stagger — blocks stay hidden until their own delayed start. The `.page-enter` wrapper is `display: contents`, so the `.app-content` flex gap is untouched (layout byte-identical). Under `prefers-reduced-motion` everything is `animation: none` → instant render; the Playwright suite (which emulates reduce) stayed fully deterministic — all **38 e2e tests green**. The animated path was verified in headless Chromium via computed styles (wrapper `display: contents`, staggered `page-pop` on portals/stats children + header title, `none` under reduce). Gates: tsc/oxlint/prettier, 119 Vitest (no new tests — CSS-only, no JS surface), build.
...
- **`recommended_action` scoring**: `Portal.recommended_action` sums condition points per branch (no-observer: STABILIZE +1 if stability < 50, CLOSE +10 if empty, SEND_OBSERVER +1 if TTL < 5 min; observer-inside: STABILIZE +1, RECALL_OBSERVER +1 if TTL < 30 s, WARN_CREATURES +1 if TTL < 5 min, RECALL_OBSERVER +10 if empty), max wins, ties by CLOSE > SEND > WARN > RECALL > STABILIZE > DISMISS, empty map or closed portal → DISMISS. CRITICAL portals bypass the score with a fixed 100-point table (observer + creatures → WARN_CREATURES, observer without creatures → RECALL_OBSERVER, no observer → CLOSE). Exposed on `PortalSchema` (list/detail/action response), so it updates after every click. Tests: full scoring matrix incl. 10-point dominance, every tie, fallback, closed/expired, and the CRITICAL table.
