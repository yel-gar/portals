...
The background effect now frames the **whole app** instead of the content column: the spark layer moved from `.app-main` to the `.app-shell` root (spans the sidebar too), and the main UI surfaces became **slightly transparent** (plain rgba — user chose alpha over frosted-glass blur): sidebar gradient `rgba(21,13,5,0.86)→rgba(10,6,3,0.86)`, header `rgba(18,11,7,0.84)`, cards/table/inputs via `colorBgContainer: rgba(23,16,8,0.82)` in `theme.ts`. Dialogs keep the opaque elevated background (`colorBgElevated` — modal, dropdowns, messages) for readability. Gates green: tsc/oxlint/prettier, 119 Vitest, build; 38-test Playwright suite green; SwiftShader smoke on the rebuilt e2e stack re-verified the full-width canvas.
