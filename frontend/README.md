...
- не считает риск/уровень угрозы и не проверяет допустимость действий — оценки и
...
- Модалка портала: справа — collapsible-панель «История портала» (открыта по
  умолчанию, схлопывание/раскрытие — CSS-анимация `max-width`/`opacity`, модалка при этом
  расширяется наружу 720px → 1040px). Панель показывает 5 последних записей
  (`GET /portals/log?portal_id=N`, обновляется после каждого действия через общую
  инвалидацию `["portals"]` + 30 c poll) и ссылку «Полная история портала» на `/log`,
  предфильтрованный к этому порталу (`/log?portal_id=N` читается при монтировании страницы
  журнала и сбрасывается кнопкой «Показать все»).
- Рекомендованное действие (`recommended_action` из `PortalSchema`, считается сервером):
  кнопка подсвечена пульсирующим фиолетовым контуром (`.portal-action-recommended`,
  под reduced motion — статичный контур), в подписи пояснено, что фиолетовый —
  рекомендованное действие. После каждого действия сервер присылает свежий портал с
  пересчитанной рекомендацией (`onPortalUpdated`), подсветка переезжает без доп. запроса.
- Принудительное закрытие: CLOSE на критическом портале с существами внутри сначала
  показывает confirm-модалку с предупреждением и уходит с `force=true`; на некритическом
...
- Декоративная фоновая подложка (`src/components/fire/`): WebGPU-шейдер магических
  потоков с мелкой искрящейся пылью (TypeGPU, TS-first WGSL, код-сплит chunk — тянется
  только когда `navigator.gpu` доступен), покрывает весь фон приложения, включая сайдбар;
  константы эффекта — в `fireTuning.ts`. Без WebGPU — 2D-canvas-эмбер-фолбэк с восходящими
  искрами, а при `prefers-reduced-motion` — статичный градиент без анимационного цикла.
  Ручной выбор бэкенда для отладки — query-параметр `?fire=webgpu|ember|static`.
  Слой фиксированный на вьюпорт с `z-index: -1`, `pointer-events: none`, `pointer-events: none`,
