...
- Декоративная фоновая подложка (`src/components/fire/`): WebGPU-шейдер искрящихся
  ...
  анимационного цикла. На Windows всегда используется эмбер-фолбэк (у шейдера там
  неподтверждённые драйверные артефакты). Слой фиксированный на вьюпорт с `z-index: -1`, `pointer-events: none`,
